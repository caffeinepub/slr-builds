...
actor {
  ...
  /// Seed test data — split into 6 functions to avoid ICP instruction limit

  public shared func seedSkillsAndBranches() : async () {
    let skillsData = [
      { id = 1; name = "РАНА"; rarity = "basic"; imageUrl = "https://say-gg.ru/static/branches/wound.png" },
      { id = 2; name = "ЗАМОРОЗКА"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/freeze.png" },
      { id = 3; name = "ЯРОСТЬ"; rarity = "legendary"; imageUrl = "https://say-gg.ru/static/branches/rage.png" },
      { id = 4; name = "ИСЦЕЛЕНИЕ"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/heal.png" },
      { id = 5; name = "ЯД"; rarity = "basic"; imageUrl = "https://say-gg.ru/static/branches/poison.png" },
      { id = 6; name = "ЩИТ"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/shield.png" },
      { id = 7; name = "КРИТ"; rarity = "legendary"; imageUrl = "https://say-gg.ru/static/branches/crit.png" },
      { id = 8; name = "СПРАЙТ"; rarity = "basic"; imageUrl = "https://say-gg.ru/static/branches/sprite.png" },
      { id = 9; name = "ХП"; rarity = "basic"; imageUrl = "https://say-gg.ru/static/branches/hp.png" },
      { id = 10; name = "УЛЬТ"; rarity = "legendary"; imageUrl = "https://say-gg.ru/static/branches/ultimate.png" },
      { id = 11; name = "АТАКА"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/attack.png" },
      { id = 12; name = "ДОДЖ"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/dodge.png" },
      { id = 13; name = "УСКОРЕНИЕ"; rarity = "basic"; imageUrl = "https://say-gg.ru/static/branches/haste.png" },
    ];
    skillStore.clear();
  ...
  public shared func seedHeroes() : async () {
    let heroNames : [Text] = [
      ...
      "Золотой Воин", "Красный Страж", "Гоблин-Плут", "Пингвин", "Тёмный Убийца",
      "Белый Медведь", "Рыжая Тень", "Паук", "Нага", "Хищник",
      "Двуглавый", "Фантом", "Эльф-Лучник", "Гоблин-Механик", "Древний Тролль",
      "Красный Демон", "Минотавр", "Лесная Фея", "Рыцарь", "Древень",
      "Тёмный Властелин", "Пиратка", "Тёмный Волшебник", "Ящер", "Оборотень",
      "Викинг", "Гремлин", "Серый Волк", "Кузнец", "Синяя Тварь",
      "Лозоход", "Гоблинша", "Панда", "Пират", "Пурпурный Гоблин",
      "Огненный Воин", "Теневой Убийца", "Гоблин-Алхимик", "Крысолов", "Анубис",
      "Зелёный Гоблин", "Синий Дракон", "Ледяная Дева", "Упырь", "Кристальный Голем",
      "Стальной Ковбой", "Изобретатель", "Огненная Демоница", "Белый Рыцарь", "Чума"
    ];
    heroStore.clear();
    var heroIdx = 0;
    while (heroIdx < 65) {
  ...
  public shared func seedItemsA() : async () {
    let names : [Text] = [
    ...
    var id = 1;
    while (id <= 50) {
      let imageUrl = "https://say-gg.ru/static/items/" # id.toText() # ".jpg";
      itemStore.add(id, { id; name = names[id - 1]; imageUrl });
      id += 1;
    };
  };

  public shared func seedItemsB() : async () {
    let names : [Text] = [
    ...
    while (id <= 99) {
      let imageUrl = "https://say-gg.ru/static/items/" # id.toText() # ".jpg";
      itemStore.add(id, { id; name = names[id - 51]; imageUrl });
      id += 1;
    };
  };

  public shared func seedBuildsA() : async () {
    buildStore.clear();
    let data : [(Nat, Text, [Nat], [Nat], [Nat], Text, Nat, Nat, Nat, Nat)] = [
      (1,  "ЛУЧНИЦА ХЕЙСТ КРИТ УЛЬТ",   [9,13],  [7,13,10,11], [4,9],   "Лучник с ускорением, критом и ультой. Быстрый дальний урон.",    2, 2, 2, 7),
      (2,  "СПРАЙТЫ УРОН",              [6,8],   [8,11,7],     [4,6],   "Спрайты усиливают атаку — огромный ДПС.",                       0, 2, 3, 6),
      ...
      (5,  "СПРАЙТ ПРИСТ",              [8,46],  [8,4,9],      [3,7],   "Спрайт-жрец: живучесть через лечение и HP.",                    0, 2, 4, 10),
      (6,  "СНАЙПЕР УЛЬТ",              [13,1],  [10,7,13],    [4,6],   "Снайпер наносит ульт-урон через крит и ускорение.",             2, 2, 1, 7),
      (7,  "КРАТОС",                    [1,30],  [3,11,7],     [4,9],   "Кратос: ярость, атака, крит. Агрессивный стиль.",               2, 3, 1, 7),
      (8,  "EVIL BRAKER ULT",           [16,12], [10,7,12],    [4,9],   "Разрушитель: ульт, крит и уклон. Контрит любого врага.",        2, 3, 1, 8),
      (9,  "ЯРОСТЬ",                    [7,4],   [3,11,1],     [4,6],   "Чистая ярость: рана и атака для быстрой победы.",              1, 2, 2, 7),
      (10, "ПУДЖ-МЕДИКАЛ",              [2,21],  [4,9,6],      [3,7],   "Пудж-медик: живёт вечно через щит, HP и лечение.",             1, 3, 3, 12),
      (11, "КРИТ-ДОДЖ",                 [5,18],  [7,12,11],    [4,9],   "Криты и уклонение — классическая ДПС-уклонистая сборка.",      2, 3, 1, 8),
      (12, "ВАНГВАРДНАЯ АТАКА",         [30,52], [11,13,3],    [4,6],   "Авангард атака с ускорением и яростью.",                       1, 2, 3, 7),
      (13, "ОГР УЛЬТ КРИТ",             [11,1],  [10,7,9],     [5,1],   "Огр: ульт с критом и HP. Живёт долго и бьёт сильно.",          2, 2, 2, 9),
      (14, "ФРОСТ ЛОРД УЛЬТ",           [3,24],  [2,10,6],     [3,7],   "Ледяной Лорд замораживает и ультует. Контроль + выживаемость.", 2, 3, 1, 11),
      (15, "СПРАЙТ ФРИЗ АТАКА",         [8,2],   [8,2,11],     [4,9],   "Спрайт + заморозка + атака. Яд и заморозка — DoT.",            1, 2, 3, 8),
      (16, "ПЕРЕЦ ЯРОСТЬ РАНЫ",         [14,9],  [3,1,11],     [4,6],   "Ярость с ранами — быстрый агрессивный стиль.",                 1, 2, 2, 7),
      (17, "ЖИВУЧЕСТЬ ТАЙМКУРС",        [2,21],  [4,9,6],      [3,7,1], "Максимальная живучесть: щит+HP+лечение.",                      1, 3, 4, 14),
      (18, "AXE COMBO ULT",             [19,30], [10,11,3],    [4,6],   "Топорщик: комбо ульт+ярость. Мощный выброс урона.",            2, 2, 2, 7),
      (19, "РАКЕТЫ ХЕЙСТ",              [25,19], [13,11,7],    [4,6],   "Ракетчик с ускорением: быстрые критические удары.",            1, 2, 3, 7),
      (20, "РАНЫ ЩИТЫ ЦИКЛИЧНОСТЬ",     [9,45],  [1,6,9],      [3,7],   "Цикл: рана+щит+HP. Пережить врага и победить.",                0, 2, 4, 11),
      (21, "ФРИЗ ПРОКИ КРИТ",           [3,24],  [2,7,11],     [4,9],   "Заморозка затем крит. Контроль + урон.",                       2, 3, 1, 8),
    ];
    for ((bId, bName, bHeroIds, bReqSkills, bForbSkills, bHint, bCostLeg, bCostRare, bCostBasic, bRounds) in data.values()) {
      buildStore.add(bId, {
        id = bId; name = bName; authorId = Principal.fromText("2vxsx-fae");
        heroIds = bHeroIds; requiredSkillIds = bReqSkills; forbiddenSkillIds = bForbSkills;
        hint = bHint; costLegendary = bCostLeg; costRare = bCostRare; costBasic = bCostBasic;
        rounds = bRounds; isPublic = true; createdAt = Time.now();
      });
    };
  };

  public shared func seedBuildsB() : async () {
    let data : [(Nat, Text, [Nat], [Nat], [Nat], Text, Nat, Nat, Nat, Nat)] = [
      (22, "ПИНГВИН ЯРОСТЬ УЛЬТ ФРИЗ",  [10,3],  [3,10,2],     [4,6],   "Пингвин: ярость+ульт+заморозка. Замораживает и уничтожает.",   2, 2, 2, 8),
      (23, "МЕРТВЫЕ СПРАЙТЫ",           [8,6],   [8,5,1],      [4,6],   "Мёртвые спрайты: яд+рана толкают DoT до предела.",             0, 1, 4, 6),
      (24, "ХП",                        [2,45],  [9,4,6],      [3,7],   "Максимальный HP: щит+лечение держат вас в живых.",             1, 3, 3, 12),
      (25, "АСТРАЛ УЛЬТ",               [15,20], [10,8,13],    [1,5],   "Астральный: спрайт+ускорение дают ульт каждый раунд.",         2, 2, 2, 7),
      (26, "АТАК КРИТ",                 [1,11],  [11,7,13],    [4,6],   "Атака+крит+ускорение — стабильный урон каждый раунд.",         1, 2, 3, 8),
      (27, "ЭКСОДИЯ",                   [23,1],  [3,7,10,2],   [4,9],   "Экзодия: ярость+крит+ульт+заморозка. Самое мощное комбо.",     3, 3, 1, 7),
      (28, "ДОДЖ",                      [5,12],  [12,11,13],   [4,9],   "Чистое уклонение: уклон+атака+ускорение. Почти неуязвим.",     1, 3, 2, 8),
      (29, "МАТХЕР",                    [22,1],  [3,11,7],     [2,4],   "Матрица: ярость+атака+крит без заморозки и лечения.",          2, 2, 2, 7),
      (30, "ОВЕРЛОРД СПРАЙТЫ УЛЬТ",     [20,8],  [8,10,3],     [4,9],   "Повелитель+спрайты+ульт — взрывной урон.",                     2, 2, 2, 8),
      (31, "БЕРСЕРК СТИЛИТ ХП",         [4,9],   [3,9,11],     [2,6],   "Берсерк крадёт HP: ярость+HP баланс атаки/выживаемости.",      1, 2, 3, 9),
      (32, "SIN ONESHOT",               [18,1],  [7,10,3],     [4,6,9], "Убийца: одним ударом через крит+ульт+ярость.",                 3, 2, 1, 6),
      (33, "DUKE DODGE",                [5,12],  [12,3,11],    [4,9],   "Герцог: уклон+ярость+атака. Быстрый и неуловимый.",            1, 3, 1, 7),
      (34, "ПАУК ХЕЙСТ",                [6,25],  [13,5,1],     [2,6],   "Паук с ускорением: яд+рана через скорость.",                   0, 1, 4, 6),
      (35, "ФРОСТ МАГИЧКА КРИТ УЛЬТ",   [8,24],  [2,7,10],     [3,9],   "Ледяной маг: крит+ульт+заморозка. Контроль и мощь.",           2, 3, 1, 9),
      (36, "ДРУАДА ДЕФ ЩИТЫ",           [17,46], [10,6,4],     [3,7],   "Друид: защита через ульт+щиты+лечение. Строение-крепость.",    0, 3, 3, 13),
      (37, "ONE BRANCH DODGE",          [6,28],  [11,5,13],    [4,9],   "Нага: атака+яд+ускорение. Постепенный смертельный урон.",       0, 2, 4, 8),
      (38, "ONE BRANCH SPRITE",         [9,20],  [10,8,3],     [5,1],   "Гарем ульт: спрайт+ярость+ульт. Максимальный выброс урона.",   2, 2, 2, 7),
      (39, "ONE BRANCH ATACK",          [11,34], [3,12,7],     [4,9],   "Чёрный Рыцарь: ярость+крит+уклон. Непобедимый рыцарь смерти.",2, 3, 1, 8),
      (40, "FROST NOVA LORD",           [11,2],  [4,9,6],      [3,7],   "Маг-Пудж: максимальная живучесть через щит+HP+лечение.",       1, 3, 3, 11),
      (41, "СНАЙПЕР АТАКА КРИТ",        [46,6],  [5,2,9],      [3,7],   "Лозоход: яд+заморозка+HP. Медленная и верная смерть.",         0, 2, 4, 12),
      (42, "ФЬЮРИ ЯРОСТЬ",              [32,1],  [3,7,10,12],  [4,9],   "Пандемониум: ярость+крит+ульт+уклон. Хаос unleashed.",         3, 3, 2, 8),
    ];
    for ((bId, bName, bHeroIds, bReqSkills, bForbSkills, bHint, bCostLeg, bCostRare, bCostBasic, bRounds) in data.values()) {
      buildStore.add(bId, {
        id = bId; name = bName; authorId = Principal.fromText("2vxsx-fae");
  ...
  /// Legacy combined seed — calls all 6 parts sequentially
