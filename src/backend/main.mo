...
actor {
  ...
  public shared func seedBuilds() : async () {
    ...
    let seedBuildsData : [(Nat, Text, [Nat], [Nat], [Nat], Text, Nat, Nat, Nat, Nat)] = [
      ...
      (8,  "EVIL BRAKER ULT + MAG CRIT + DODGE",  [16,12], [10,7,12],    [4,9],    "Разрушитель: ульт, крит и уклон. Контрит любого врага.",     2, 3, 1, 8),
      ...
      (15, "СПРАЙТ ФРИЗ АТАКА, ЯД+ФРИЗ",        [8,2],   [8,2,11],     [4,9],    "Спрайт + заморозка + атака. Яд и заморозка дают огромный DoT.",      1, 2, 3, 8),
      (16, "ПЕРЕЦ ЯРОСТЬ, РАНЫ",                 [14,9],  [3,1,11],     [4,6],    "Ярость с ранами — быстрый агрессивный стиль.",                   1, 2, 2, 7),
      (17, "ЖИВУЧЕСТЬ+ТАЙМКУРС",         [2,21],  [4,9,6],      [3,7,1],  "Максимальная живучесть: щит+HP+лечение. Победа на измоте.",      1, 3, 4, 14),
      (18, "AXE_COMBO_ULT",               [19,30], [10,11,3],    [4,6],    "Топорщик: комбо ульт+ярость. Мощный выброс урона.",              2, 2, 2, 7),
      ...
      (36, "ДРУАДА ДЕФ ЧЕРЕЗ УЛЬТ, ЩИТЫ",       [17,46], [10,6,4],     [3,7],    "Друид: защита через ульт+щиты+лечение. Строение-крепость.",        0, 3, 3, 13),
