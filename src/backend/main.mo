...
actor {
  ...
  public shared func seedSkillsAndBranches() : async () {
    let skillsData = [
      ...
      { id = 9; name = "HP"; rarity = "basic"; imageUrl = "https://say-gg.ru/static/branches/hp.png" },
      { id = 10; name = "ULT"; rarity = "legendary"; imageUrl = "https://say-gg.ru/static/branches/ultimate.png" },
      { id = 11; name = "АТАКА"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/attack.png" },
      { id = 12; name = "УКЛОНЕНИЕ"; rarity = "rare"; imageUrl = "https://say-gg.ru/static/branches/dodge.png" },
  ...
  public shared func seedBuilds() : async () {
    ...
    let seedBuildsData : [(Nat, Text, [Nat], [Nat], [Nat], Text, Nat, Nat, Nat, Nat)] = [
      (1,  "Лучник: Ускорение + Крит + Ульт",        [9,13],  [7,13,10,11], [4,9],    "Лучник с ускорением, критом и ультой. Быстрый дальний урон.",       2, 2, 2, 7),
      (2,  "Спрайт: ДПС",                   [6,8],   [8,11,7],     [4,6],    "Спрайты усиливают атаку — огромный ДПС.",                        0, 2, 3, 6),
      (3,  "Старт в лобби",             [1,3],   [13,11,8],    [2,6],    "Быстрый старт через атаку и ускорение до битвы.",             0, 2, 3, 5),
      (4,  "Маг: Крит + Панда",              [12,10], [7,10,11],    [4,6,9],  "Маг с критом — максимальный урон без защиты.",                    2, 2, 2, 8),
      (5,  "Спрайт-Жрец",                [8,46],  [8,4,9],      [3,7],    "Спрайт-жрец: живучесть через лечение и HP.",               0, 2, 4, 10),
      (6,  "Снайпер: Ульт",                   [13,1],  [10,7,13],    [4,6],    "Снайпер наносит ульт-урон через крит и ускорение.",              2, 2, 1, 7),
      (7,  "Кратос",                       [1,30],  [3,11,7],     [4,9],    "Кратос: ярость, атака, крит. Агрессивный стиль для первого места.",   2, 3, 1, 7),
      (8,  "Разрушитель: Ульт + Крит + Уклон",  [16,12], [10,7,12],    [4,9],    "Разрушитель: ульт, крит и уклон. Контрит любого врага.",     2, 3, 1, 8),
      (9,  "Чистая Ярость",                   [7,4],   [3,11,1],     [4,6],    "Чистая ярость: рана и атака для быстрой победы.",               1, 2, 2, 7),
      (10, "Пудж-Медик",                [2,21],  [4,9,6],      [3,7],    "Пудж-медик: живёт вечно через щит, HP и лечение.",         1, 3, 3, 12),
      (11, "Крит + Уклон",                   [5,18],  [7,12,11],    [4,9],    "Криты и уклонение — классическая ДПС-уклонистая сборка.",               2, 3, 1, 8),
      (12, "Авангард: Атака",              [30,52], [11,13,3],    [4,6],    "Авангард атака с ускорением и яростью.",                       1, 2, 3, 7),
      (13, "Огр: Ульт + Крит",               [11,1],  [10,7,9],     [5,1],    "Огр: ульт с критом и HP. Живёт долго и бьёт сильно.",       2, 2, 2, 9),
      (14, "Ледяной Лорд: Ульт",              [3,24],  [2,10,6],     [3,7],    "Ледяной Лорд замораживает и ультует. Контроль + выживаемость.",           2, 3, 1, 11),
      (15, "Спрайт + Заморозка",        [8,2],   [8,2,11],     [4,9],    "Спрайт + заморозка + атака. Яд и заморозка дают огромный DoT.",      1, 2, 3, 8),
      (16, "Ярость + Раны",                 [14,9],  [3,1,11],     [4,6],    "Ярость с ранами — быстрый агрессивный стиль.",                   1, 2, 2, 7),
      (17, "Танк: Максимальная живучесть",         [2,21],  [4,9,6],      [3,7,1],  "Максимальная живучесть: щит+HP+лечение. Победа на измоте.",      1, 3, 4, 14),
      (18, "Топор: Комбо + Ульт",               [19,30], [10,11,3],    [4,6],    "Топорщик: комбо ульт+ярость. Мощный выброс урона.",              2, 2, 2, 7),
      (19, "Ракетчик: Ускорение",                [25,19], [13,11,7],    [4,6],    "Ракетчик с ускорением: быстрые критические удары.",                1, 2, 3, 7),
      (20, "Цикл: Рана + Щит",          [9,45],  [1,6,9],      [3,7],    "Цикл: рана+щит+HP. Пережить врага и победить.",          0, 2, 4, 11),
      (21, "Заморозка + Крит",            [3,24],  [2,7,11],     [4,9],    "Заморозка затем крит. Контроль + урон.",                    2, 3, 1, 8),
      (22, "Пингвин: Ярость + Ульт + Заморозка",     [10,3],  [3,10,2],     [4,6],    "Пингвин: ярость+ульт+заморозка. Замораживает и уничтожает.",             2, 2, 2, 8),
      (23, "Мёртвые Спрайты",                [8,6],   [8,5,1],      [4,6],    "Мёртвые спрайты: яд+рана толкают DoT до предела.",          0, 1, 4, 6),
      (24, "Максимальный HP",                      [2,45],  [9,4,6],      [3,7],    "Максимальный HP: щит+лечение держат вас в живых.",                     1, 3, 3, 12),
      (25, "Астральный Ульт",                  [15,20], [10,8,13],    [1,5],    "Астральный: спрайт+ускорение дают ульт каждый раунд.",                  2, 2, 2, 7),
      (26, "Атака + Крит",                 [1,11],  [11,7,13],    [4,6],    "Атака+крит+ускорение — стабильный урон каждый раунд.",              1, 2, 3, 8),
      (27, "Экзодия",                      [23,1],  [3,7,10,2],   [4,9],    "Экзодия: ярость+крит+ульт+заморозка. Самое мощное комбо.",          3, 3, 1, 7),
      (28, "Мастер Уклонения",               [5,12],  [12,11,13],   [4,9],    "Чистое уклонение: уклон+атака+ускорение. Почти неуязвим.",        1, 3, 2, 8),
      (29, "Матрица",                      [22,1],  [3,11,7],     [2,4],    "Матрица: ярость+атака+крит без заморозки и лечения.",                 2, 2, 2, 7),
      (30, "Повелитель: Спрайты + Ульт",        [20,8],  [8,10,3],     [4,9],    "Повелитель+спрайты+ульт — взрывной урон.",                    2, 2, 2, 8),
      (31, "Берсерк: HP",                [4,9],   [3,9,11],     [2,6],    "Берсерк крадёт HP: ярость+HP баланс атаки/выживаемости.",   1, 2, 3, 9),
      (32, "Убийца: Одним Ударом",                 [18,1],  [7,10,3],     [4,6,9],  "Убийца: одним ударом через крит+ульт+ярость.",              3, 2, 1, 6),
      (33, "Герцог: Уклон",                  [5,12],  [12,3,11],    [4,9],    "Герцог: уклон+ярость+атака. Быстрый и неуловимый.",                  1, 3, 1, 7),
      (34, "Паук: Ускорение",                [6,25],  [13,5,1],     [2,6],    "Паук с ускорением: яд+рана через скорость.",                   0, 1, 4, 6),
      (35, "Ледяной Маг: Крит + Ульт",         [8,24],  [2,7,10],     [3,9],    "Ледяной маг: крит+ульт+заморозка. Контроль и мощь.",             2, 3, 1, 9),
      (36, "Друид: Защита + Щиты",       [17,46], [10,6,4],     [3,7],    "Друид: защита через ульт+щиты+лечение. Строение-крепость.",        0, 3, 3, 13),
      (37, "Нага: Комбо",                  [6,28],  [11,5,13],    [4,9],    "Нага: атака+яд+ускорение. Постепенный смертельный урон.",           0, 2, 4, 8),
      (38, "Гарем: Ульт",                   [9,20],  [10,8,3],     [5,1],    "Гарем ульт: спрайт+ярость+ульт. Максимальный выброс урона.",           2, 2, 2, 7),
      (39, "Чёрный Рыцарь",                [11,34], [3,12,7],     [4,9],    "Чёрный Рыцарь: ярость+крит+уклон. Непобедимый рыцарь смерти.",    2, 3, 1, 8),
      (40, "Маг-Пудж: Живучесть",             [11,2],  [4,9,6],      [3,7],    "Маг-Пудж: максимальная живучесть через щит+HP+лечение.",           1, 3, 3, 11),
      (41, "Лозоход: Яд + Заморозка",      [46,6],  [5,2,9],      [3,7],    "Лозоход: яд+заморозка+HP. Медленная и верная смерть.",         0, 2, 4, 12),
      (42, "Пандемониум",                 [32,1],  [3,7,10,12],  [4,9],    "Пандемониум: ярость+крит+ульт+уклон. Хаос unleashed.",          3, 3, 2, 8),
    ];
